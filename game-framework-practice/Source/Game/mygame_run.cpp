...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	if (phase == 3)
	{
		if (abs(character.Top() - 430) < 50 && abs(character.Left() - 150) < 50)
		{
			//void  LoadBitmap(int, COLORREF = CLR_INVALID);		// 載入圖，指定圖的編號(resource)及透明色
			//chest_and_key.SetTopLeft(0, 0);
			//chest_and_key.LoadBitmapA(0,0);
			//chest_and_key.LoadBitmapByString({ "resources/chest.bmp", "resources/chest_ignore.bmp" }, RGB(100, 0, 0));
			//chest_and_key.SetTopLeft(15, 430);
	...
	for (int i = 0; i < 3; i++) {
		if (phase == 5)
		{
			if (abs(character.Top() - 265) < 50 && abs(character.Left() - 462 + 100 * i) < 50)
			{
